...
ANTIPATTERNS = [
    {"pattern": "skip_why", "symptom": "Jumping to solution without asking WHY", "fix": "Always ask why before how"},
    {"pattern": "hidden_assumptions", "symptom": "Not surfacing what you're assuming", "fix": "Explicitly state and validate assumptions"},
    {"pattern": "surface_accept", "symptom": "Accepting request at face value", "fix": "Dig deeper with laddering/5 whys"},
    {"pattern": "no_confirm", "symptom": "Proceeding without explicit confirmation", "fix": "Get explicit 'yes, proceed'"},
    {"pattern": "over_question", "symptom": "Too many questions for simple requests", "fix": "Match depth to complexity"},
    {"pattern": "leading_questions", "symptom": "Questions that assume the answer", "fix": "Use open, neutral questions"},
    {"pattern": "ignore_context", "symptom": "Not using available information", "fix": "Review context before asking"},
]

# Request complexity signals
COMPLEXITY_SIGNALS = {
    "high": ["multiple", "integrate", "migrate", "architecture", "redesign", "complex", "enterprise", "scale"],
...
SOLUTION_INDICATORS = ["use", "add", "implement", "create", "build", "make", "put", "install"]
INTENT_INDICATORS = ["need", "want", "problem", "issue", "help", "struggling", "goal", "trying"]
...
class AnalyzeRequestInput(BaseModel):
    model_config = _CFG
    request: str = Field(..., min_length=1, max_length=5000, description="User request to analyze")
...
class GenerateWhyInput(BaseModel):
    ...
    technique: str = Field(default="mixed", description="Technique: laddering, five_whys, or mixed")
...
class SurfaceAssumptionsInput(BaseModel):
    model_config = _CFG
    request: str = Field(..., min_length=1, max_length=2000, description="User request")
    domain: str = Field(default="software", description="Domain: software, document, automation, general")
    existing_context: dict[str, str] = Field(default_factory=dict, description="Known context")


class GenerateOptionsInput(BaseModel):
    model_config = _CFG
    intent: str = Field(..., min_length=1, max_length=500, description="Discovered intent (WHY)")
    constraints: list[str] = Field(default_factory=list, description="Known constraints")
    num_options: int = Field(default=3, ge=2, le=5, description="Number of options to generate")


class GenerateSummaryInput(BaseModel):
    model_config = _CFG
    problem_why: str = Field(..., min_length=1, max_length=500, description="Problem/intent (WHY)")
    solution_what: str = Field(..., min_length=1, max_length=500, description="Solution (WHAT)")
    key_decisions: list[str] = Field(default_factory=list, description="Key decisions made")
    out_of_scope: list[str] = Field(default_factory=list, description="What's not included")
    format: str = Field(default="standard", description="Format: quick, standard, or detailed")
...
class ValidateDepthInput(BaseModel):
    model_config = _CFG
    understanding: dict = Field(..., description="Current understanding {why, what, assumptions, scope}")


class DetectAntipatternsInput(BaseModel):
    model_config = _CFG
    conversation: str = Field(..., min_length=10, max_length=10000, description="Conversation to analyze")


class ClassifyRequestInput(BaseModel):
    model_config = _CFG
    request: str = Field(..., min_length=1, max_length=2000, description="User request to classify")


class GenerateProbesInput(BaseModel):
    model_config = _CFG
    text: str = Field(..., min_length=1, max_length=2000, description="Text containing vague terms")
    max_probes: int = Field(default=3, ge=1, le=5, description="Maximum probes to generate")


class GenerateConfirmationInput(BaseModel):
    model_config = _CFG
    understanding: str = Field(..., min_length=1, max_length=1000, description="Summary of understanding")
    next_action: str = Field(..., min_length=1, max_length=500, description="What will happen next")
...
def _detect_solution_vs_intent(text: str) -> dict:
    ...
    if solution_score > intent_score:
        return {"type": "solution", "recommendation": "Dig deeper to discover intent behind the solution"}
    elif intent_score > solution_score:
        return {"type": "intent", "recommendation": "Good - user is expressing the problem/need"}
    else:
        return {"type": "mixed", "recommendation": "Clarify whether this is the problem or a proposed solution"}
...
def _assess_complexity(text: str) -> dict:
    ...
    scores = {level: sum(1 for kw in keywords if kw in text_lower) for level, keywords in COMPLEXITY_SIGNALS.items()}
...
def _generate_laddering_questions(request: str) -> list[dict]:
    ...
    return [
        {"question": f'When you say "{request[:50]}...", what problem does this solve?', "purpose": "Surface real need"},
        {"question": "Why is that important for you/your users?", "purpose": "Dig into importance"},
        {"question": "What happens if this problem isn't solved?", "purpose": "Understand stakes"},
        {"question": "What does the ideal outcome look like?", "purpose": "Define success"},
    ]
...
def _generate_five_whys(request: str) -> list[dict]:
    ...
    return [
        {"level": 1, "question": "Why do you need this?", "purpose": "First level intent"},
        ...
        {"level": 5, "question": "Why is this the approach?", "purpose": "Root cause/need"},
    ]
...
async def interview_analyze_request(
    ...
    try:
        ...
        ambiguities = []
        if vague_terms:
            ambiguities.append(f"Contains {len(vague_terms)} vague term(s) needing clarification")
        if solution_intent["type"] == "solution":
            ambiguities.append("States a solution without expressing the underlying problem")
...
async def interview_surface_assumptions(
    ...
    try:
        ...
        if inp.domain == "software":
            assumptions.extend([
                {"category": "context", "assumption": "This is for a web application", "validate": "Is this web, mobile, desktop, or API?"},
                {"category": "users", "assumption": "Users are moderately technical", "validate": "Who are the target users?"},
                {"category": "scale", "assumption": "Standard scale (100s-1000s of users)", "validate": "What scale are we building for?"},
                {"category": "integration", "assumption": "Standalone feature", "validate": "Does this integrate with existing systems?"},
            ])
        elif inp.domain == "document":
            assumptions.extend([
                {"category": "context", "assumption": "Single document/artifact", "validate": "Is this one document or a template?"},
                {"category": "users", "assumption": "Internal audience", "validate": "Who will read this?"},
                {"category": "quality", "assumption": "Professional tone", "validate": "What tone/style is appropriate?"},
            ])
        else:
            assumptions.extend([
                {"category": "context", "assumption": "Current environment/tools", "validate": "What environment/tools are we working with?"},
                {"category": "scope", "assumption": "Single task", "validate": "Is this part of something larger?"},
            ])
...
async def interview_generate_options(
    ...
    try:
        inp = GenerateOptionsInput(intent=intent, constraints=constraints or [], num_options=num_options)
...
Proceeding with this approach. Confirm?'''
            ...
            decisions_text = "\n".join([f"- {d}" for d in inp.key_decisions]) if inp.key_decisions else "- None specified"
            scope_text = "\n".join([f"- {s}" for s in inp.out_of_scope]) if inp.out_of_scope else "- Nothing explicitly excluded"
...
async def interview_validate_depth(
    ...
    try:
        ...
        checklist = [
            {"item": "Know WHY (intent/problem)", "check": bool(u.get("why")), "critical": True},
            {"item": "Know WHAT (solution)", "check": bool(u.get("what")), "critical": True},
            {"item": "Assumptions surfaced", "check": bool(u.get("assumptions")), "critical": True},
            {"item": "Assumptions validated", "check": u.get("assumptions_validated", False), "critical": True},
            {"item": "Know who benefits", "check": bool(u.get("beneficiaries")), "critical": False},
            {"item": "Scope boundaries clear", "check": bool(u.get("scope") or u.get("out_of_scope")), "critical": False},
            {"item": "User confirmed understanding", "check": u.get("confirmed", False), "critical": True},
        ]
        ...
        critical_missing = [c["item"] for c in checklist if c["critical"] and not c["check"]]
        ...
        return json.dumps({
            ...
            "recommendation": "Proceed with implementation" if ready else f"Address: {', '.join(critical_missing[:2])}",
        })
    except ValueError as e:
        return json.dumps({"error": str(e)})
...
async def interview_detect_antipatterns(
    ...
    try:
        ...
        if any(s in conv_lower for s in ["let me", "i'll create", "i'll implement"]) and "why" not in conv_lower:
            findings.append({
                "pattern": "skip_why",
        ...
        if "proceed" in conv_lower and not any(c in conv_lower for c in ["confirm", "correct?", "okay?", "yes"]):
            findings.append({
                "pattern": "no_confirm",
        ...
        return json.dumps({
            ...
            "count": len(findings),
            "severity": severity,
            "recommendation": findings[0]["fix"] if findings else "Discovery process looks good",
        })
    except ValueError as e:
        return json.dumps({"error": str(e)})
...
async def interview_classify_request(
    ...
    try:
        ...
        if needs_signals >= 3:
            ...
            recommendation = "Conduct full WHY + assumptions discovery before proceeding"
        ...
        return json.dumps({
            ...
            "suggested_first_question": WHY_QUESTIONS[0]["q"] if classification != "proceed" else None,
        })
    except ValueError as e:
        return json.dumps({"error": str(e)})
...
async def interview_generate_probes(
    ...
    try:
        ...
        if not probes:
            probes = [
                {"term": None, "probe": "Can you tell me more about what you mean?", "context": "Clarification needed"},
                {"term": None, "probe": "What would success look like here?", "context": "Defining success"},
            ]
...
async def interview_generate_confirmation(
    ...
    try:
        inp = GenerateConfirmationInput(understanding=understanding, next_action=next_action)
