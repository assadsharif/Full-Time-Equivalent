...
SELECTOR_STRATEGIES = {
    "text": {"prefix": "text=", "desc": "Match visible text content", "example": 'text="Submit"'},
    "role": {"prefix": "role=", "desc": "ARIA role selector", "example": 'role=button[name="Submit"]'},
    "css": {"prefix": "", "desc": "CSS selector", "example": "button.submit-btn"},
    "id": {"prefix": "#", "desc": "Element ID", "example": "#submit-button"},
    "data_testid": {"prefix": "[data-testid=", "desc": "Test ID attribute", "example": '[data-testid="submit"]'},
    "placeholder": {"prefix": "[placeholder=", "desc": "Input placeholder", "example": '[placeholder="Enter email"]'},
    "label": {"prefix": "label=", "desc": "Associated label text", "example": 'label="Email address"'},
}
...
ANTIPATTERNS = [
    {"pattern": r"time\.sleep\(", "issue": "Using time.sleep instead of Playwright waits", "fix": "Use page.wait_for_selector() or wait_for_load_state()"},
    {"pattern": r"headless\s*=\s*False", "issue": "Running browser in headed mode", "fix": "Use headless=True for CI/automation"},
    {"pattern": r"page\.goto\([^)]+\)\s*\n\s*page\.", "issue": "Missing wait after navigation", "fix": "Add page.wait_for_load_state('networkidle') after goto()"},
    {"pattern": r"\.click\(\)\s*\n\s*assert", "issue": "Missing wait after click", "fix": "Add wait or use expect() for assertions"},
    {"pattern": r"try:\s*\n\s*.*\n\s*except:", "issue": "Bare except clause in test", "fix": "Catch specific exceptions or use expect()"},
    {"pattern": r"page\.locator\(['\"]div['\"]\)", "issue": "Using generic 'div' selector", "fix": "Use specific selectors: data-testid, role, or text"},
    {"pattern": r"browser\.close\(\)(?!.*finally)", "issue": "browser.close() not in finally block", "fix": "Use try/finally or context manager"},
    {"pattern": r"localhost:\d+(?!/)", "issue": "Hardcoded port without trailing slash", "fix": "Consider using environment variables for URLs"},
]

# Form input types
FORM_INPUT_TYPES = ["text", "email", "password", "number", "tel", "url", "search", "date", "datetime-local", "time", "month", "week", "color", "file", "checkbox", "radio", "select", "textarea"]
...
class GenerateScriptInput(BaseModel):
    ...
    actions: list[str] = Field(default_factory=list, description="Actions to perform (click, fill, etc.)")
    headless: bool = Field(default=True, description="Run browser in headless mode")
    wait_strategy: str = Field(default="networkidle", description="Wait strategy after navigation")
    ...
    def _validate_wait(cls, v: str) -> str:
        if v not in WAIT_STRATEGIES:
            raise ValueError(f"Unknown wait strategy: {v}. Use: {list(WAIT_STRATEGIES.keys())}")
        return v
...
class GenerateDiscoveryInput(BaseModel):
    ...
    element_types: list[str] = Field(default=["button", "a", "input"], description="Element types to discover")
...
class GenerateScreenshotInput(BaseModel):
    model_config = _CFG
    url: str = Field(..., min_length=1, description="Target URL")
    output_path: str = Field(default="/tmp/screenshot.png", description="Screenshot output path")
    ...
    viewport_height: int = Field(default=720, ge=200, le=2160, description="Viewport height")
...
class GenerateFormTestInput(BaseModel):
    ...
    fields: dict[str, str] = Field(..., description="Field selectors and values to fill")
    submit_selector: str = Field(default="button[type=submit]", description="Submit button selector")
    success_indicator: Optional[str] = Field(default=None, description="Selector or text indicating success")
...
class GenerateNavigationInput(BaseModel):
    ...
    check_elements: dict[str, str] = Field(default_factory=dict, description="Route -> element selector to verify")
...
class GenerateAssertionsInput(BaseModel):
    model_config = _CFG
    url: str = Field(..., min_length=1, description="Target URL")
    assertions: list[dict] = Field(..., min_length=1, description="Assertions: [{type, selector, expected}]")
    ...
    def _validate_assertions(cls, v: list[dict]) -> list[dict]:
        valid_types = ["visible", "hidden", "text_contains", "text_equals", "count", "attribute", "enabled", "disabled"]
        for a in v:
            if "type" not in a:
                raise ValueError("Each assertion needs 'type'")
            if a["type"] not in valid_types:
                raise ValueError(f"Unknown assertion type: {a['type']}. Valid: {valid_types}")
        return v


class GenerateServerCmdInput(BaseModel):
    model_config = _CFG
    servers: list[dict] = Field(..., min_length=1, description="Server configs: [{command, port}]")
    test_script: str = Field(..., min_length=1, description="Path to test script to run")
...
class DetectAntipatternsInput(BaseModel):
    model_config = _CFG
    code: str = Field(..., min_length=10, max_length=50000, description="Test code to analyze")


class SuggestSelectorsInput(BaseModel):
    model_config = _CFG
    html: str = Field(..., min_length=10, max_length=50000, description="HTML snippet to analyze")
    target_element: Optional[str] = Field(default=None, description="Specific element to target")


class GenerateScaffoldInput(BaseModel):
    model_config = _CFG
    app_type: str = Field(..., description="Application type: spa, mpa, static")
    base_url: str = Field(default="http://localhost:3000", description="Application base URL")
    test_cases: list[str] = Field(default_factory=list, description="Test case descriptions")
    use_server_wrapper: bool = Field(default=False, description="Include with_server.py integration")
...
def _suggest_selector_for_element(tag: str, attrs: dict) -> list[dict]:
    ...
    if "data-testid" in attrs:
        suggestions.append({"selector": f'[data-testid="{attrs["data-testid"]}"]', "strategy": "data_testid", "priority": 1})

    # ID
    if "id" in attrs:
        suggestions.append({"selector": f'#{attrs["id"]}', "strategy": "id", "priority": 2})

    # Role + name
    if "role" in attrs or tag in ("button", "link", "textbox"):
        role = attrs.get("role", {"button": "button", "a": "link", "input": "textbox"}.get(tag, tag))
        name = attrs.get("aria-label", attrs.get("name", ""))
        if name:
            suggestions.append({"selector": f'role={role}[name="{name}"]', "strategy": "role", "priority": 3})

    # Text content
    if "text" in attrs:
        suggestions.append({"selector": f'text="{attrs["text"]}"', "strategy": "text", "priority": 4})

    # Placeholder
    if "placeholder" in attrs:
        suggestions.append({"selector": f'[placeholder="{attrs["placeholder"]}"]', "strategy": "placeholder", "priority": 5})

    # CSS class (lowest priority)
    if "class" in attrs:
        classes = attrs["class"].split()
        if classes:
            suggestions.append({"selector": f'{tag}.{classes[0]}', "strategy": "css", "priority": 6})
...
async def webapp_generate_script(
    ...
    try:
        inp = GenerateScriptInput(url=url, actions=actions or [], headless=headless, wait_strategy=wait_strategy)
        ...
        for i, action in enumerate(inp.actions):
            ...
            if action.startswith("click:"):
                ...
                if len(parts) == 2:
                    body_lines.append(f"            page.locator('{parts[0].strip()}').fill('{parts[1].strip()}')")
        ...
        return json.dumps({"script": script, "url": inp.url, "actions_count": len(inp.actions)})
...
async def webapp_generate_discovery(
    ...
    try:
        inp = GenerateDiscoveryInput(url=url, element_types=element_types or ["button", "a", "input"], screenshot_path=screenshot_path)
        ...
        script = header + "\n".join(body_lines) + footer
        return json.dumps({"script": script, "url": inp.url, "element_types": inp.element_types})
    except ValueError as e:
        return json.dumps({"error": str(e)})
...
async def webapp_generate_screenshot(
    ...
    try:
        inp = GenerateScreenshotInput(url=url, output_path=output_path, full_page=full_page, viewport_width=viewport_width, viewport_height=viewport_height)
...
async def webapp_generate_form_test(
    ...
    try:
        inp = GenerateFormTestInput(url=url, fields=fields, submit_selector=submit_selector, success_indicator=success_indicator)
...
async def webapp_generate_navigation(
    ...
    try:
        inp = GenerateNavigationInput(base_url=base_url, routes=routes, check_elements=check_elements or {})
        ...
        for route in inp.routes:
            full_url = f"{inp.base_url}{route}" if route.startswith("/") else f"{inp.base_url}/{route}"
...
async def webapp_generate_assertions(
    ...
    try:
        ...
        for i, assertion in enumerate(inp.assertions):
            ...
            if a_type == "visible":
                body_lines.append(f"                expect(page.locator('{selector}')).to_be_visible()")
            elif a_type == "hidden":
                body_lines.append(f"                expect(page.locator('{selector}')).to_be_hidden()")
            elif a_type == "text_contains":
                body_lines.append(f"                expect(page.locator('{selector}')).to_contain_text('{expected}')")
            elif a_type == "text_equals":
                body_lines.append(f"                expect(page.locator('{selector}')).to_have_text('{expected}')")
            elif a_type == "count":
                body_lines.append(f"                expect(page.locator('{selector}')).to_have_count({expected})")
            elif a_type == "attribute":
                attr_name = assertion.get("attribute", "")
                body_lines.append(f"                expect(page.locator('{selector}')).to_have_attribute('{attr_name}', '{expected}')")
            elif a_type == "enabled":
                body_lines.append(f"                expect(page.locator('{selector}')).to_be_enabled()")
            elif a_type == "disabled":
                body_lines.append(f"                expect(page.locator('{selector}')).to_be_disabled()")
...
async def webapp_suggest_selectors(
    ...
    try:
        ...
        button_matches = re.findall(r'<button[^>]*>([^<]+)</button>', inp.html, re.IGNORECASE)
        ...
        strategies = [
            {"strategy": s, **SELECTOR_STRATEGIES[s]}
            for s in SELECTOR_STRATEGIES
        ]
...
async def webapp_generate_scaffold(
    ...
    try:
        inp = GenerateScaffoldInput(app_type=app_type, base_url=base_url, test_cases=test_cases or [], use_server_wrapper=use_server_wrapper)
