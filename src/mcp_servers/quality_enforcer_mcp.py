...
class ClassifyInput(BaseModel):
    ...
    error_message: str = Field(..., min_length=1, description="Raw error message to classify")
...
async def quality_validate_clean(target_paths: list[str]) -> str:
    ...
    return json.dumps({
        ...
        "findings": all_findings,
        "total_errors": len(all_findings),
        "message": "CLEAN: Zero errors." if is_clean else f"NOT CLEAN: {len(all_findings)} error(s) detected.",
    })
