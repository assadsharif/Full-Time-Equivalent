...
class XeroGetInvoicesInput(BaseModel):
    ...
    status: Optional[Literal["DRAFT", "SUBMITTED", "AUTHORISED", "PAID", "VOIDED"]] = Field(
        None,
...
class XeroGetBankTransactionsInput(BaseModel):
    ...
    bank_account_id: Optional[str] = Field(
        None,
        description="Specific bank account ID (defaults to all accounts)"
    )
    from_date: Optional[str] = Field(
        None,
        description="Start date (ISO format: YYYY-MM-DD)"
...
class XeroCreateInvoiceInput(BaseModel):
    ...
    reference: Optional[str] = Field(
        None,
        description="Invoice reference/notes"
    )
...
class XeroRecordExpenseInput(BaseModel):
    ...
    date: str = Field(
        ...,
        description="Expense date (ISO format: YYYY-MM-DD)"
    )
    amount: float = Field(
        ..., gt=0,
    ...
    account_code: str = Field(
        ...,
        description="Account code for expense category"
    )
    ...
    reference: Optional[str] = Field(
        None,
        description="Expense reference/receipt number"
    )
    ...
    mock_invoices = [
        {
            ...
            "due_date": (datetime.now() - timedelta(days=i*7-30)).strftime("%Y-%m-%d"),
...
def xero_get_expenses(input: XeroGetExpensesInput) -> dict:
    ...
    return {
        ...
        "total_amount": _mock_currency_amount(sum(e["amount"]["amount"] for e in mock_expenses)),
        "mock_mode": True,
...
def xero_get_profit_loss(input: XeroGetProfitLossInput) -> dict:
    ...
    return {
        "success": True,
        "period": {
            "from_date": input.from_date or (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d"),
...
def xero_create_invoice(input: XeroCreateInvoiceInput) -> dict:
    ...
    total_amount = sum(item.get("quantity", 1) * item.get("unit_amount", 0)
    ...
    return {
        ...
        "invoice_preview": {
            "contact_id": input.contact_id,
            "invoice_number": input.invoice_number or f"INV-{datetime.now().strftime('%Y%m%d')}",
            "date": input.date,
...
def xero_get_tax_summary(input: XeroGetTaxSummaryInput) -> dict:
    ...
    return {
        "success": True,
        "period": {
            "from_date": input.from_date or (datetime.now() - timedelta(days=90)).strftime("%Y-%m-%d"),
            "to_date": input.to_date or datetime.now().strftime("%Y-%m-%d")
        },
        "tax_summary": {
