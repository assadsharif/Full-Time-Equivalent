...
class TestMCPAdd:
    ...
    @patch("cli.mcp.load_mcp_registry")
    def test_mcp_add_success(self, mock_load, mock_save, mock_checkpoint):
        ...
        result = runner.invoke(mcp_add_command, ["test-server", "https://example.com"])
    ...
    def test_mcp_add_invalid_url(self):
        ...
        result = runner.invoke(mcp_add_command, ["test-server", "ftp://invalid.com"])
    ...
    def test_mcp_add_with_auth_file(
        self, mock_load, mock_save, mock_checkpoint, tmp_path
    ):
    ...
    @patch("cli.mcp.save_mcp_registry")
    @patch("cli.mcp.load_mcp_registry")
    def test_mcp_add_duplicate_server(self, mock_load, mock_save, mock_checkpoint):
        """Test adding duplicate server updates configuration"""
        runner = CliRunner()

        # Mock registry with existing server
        mock_load.return_value = {
            "servers": {"test-server": {"url": "https://old.com", "auth": False}}
        }
        ...
        result = runner.invoke(mcp_add_command, ["test-server", "https://new.com"])
...
class TestMCPTest:
    ...
    def test_mcp_test_success(self, mock_load, mock_health):
        ...
        mock_load.return_value = {
            "servers": {"test-server": {"url": "https://example.com", "auth": False}}
        }
...
class TestMCPTools:
    ...
    @patch("cli.mcp.load_mcp_registry")
    def test_mcp_tools_success(self, mock_load, mock_get_tools, mock_save):
    ...
    @patch("cli.mcp.get_server_tools")
    @patch("cli.mcp.load_mcp_registry")
    def test_mcp_tools_empty(self, mock_load, mock_get_tools, mock_save):
